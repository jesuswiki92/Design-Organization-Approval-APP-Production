...
function buildClientRequestContext(consultation: Record<string, unknown>): string {
  ...
  if (consultation.asunto) {
    lines.push(`- Original email subject: ${consultation.asunto}`)
  }
...
export async function POST(
  ...
  try {
    ...
    if (!id) {
      return jsonResponse(400, { error: 'Consulta ID requerido.' })
    }
    ...
    if (!refs.includes(referenceProjectId)) {
      return jsonResponse(400, { error: 'El proyecto no esta marcado como referencia en esta consulta.' })
    }
    ...
    if (!project) {
      return jsonResponse(404, { error: 'Proyecto de referencia no encontrado.' })
    }

    if (!project.summary_md || !project.summary_md.trim()) {
      return jsonResponse(400, { error: 'El proyecto de referencia no tiene PROJECT_SUMMARY disponible.' })
    }
