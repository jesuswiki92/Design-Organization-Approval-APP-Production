...
function buildConsultationSummary(consultation: Record<string, unknown>): string {
  ...
  const fields: [string, string][] = [
    ['Entry number', String(consultation.entry_number ?? '')],
    ['Subject', String(consultation.subject ?? '')],
    ['Client (sender)', String(consultation.sender ?? '')],
...
const EMAIL_SYSTEM_PROMPT = [
  ...
  '- Data de peso y balance (weight & balance data)',
  ...
  '- Data de instalacion (installation data)',
  ...
  '- Data de compatibilidad electromagnetica (EMC data)',
  '- Documentacion del manufacturer del equipo (equipment manufacturer documentation)',
  '',
  'RULES FOR THE EMAIL:',
  ...
  '- Reference the consultation entry number (entry_number) in the greeting',
  ...
  '{',
  '  "subject": "string with the email subject line, must include the entry_number",',
...
export async function POST(
  ...
  try {
    ...
    const clientName = consultation.sender
      ? String(consultation.sender).split('@')[0].replace(/[._-]/g, ' ')
      : null
    const numeroEntrada = consultation.entry_number ?? consultation.codigo ?? id
    ...
    const userPrompt = [
      ...
      `Client email: ${consultation.sender ?? 'Unknown'}`,
