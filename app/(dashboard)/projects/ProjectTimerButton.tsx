 ...
 *   - "Iniciar" (verde): crea una fila new con started_at = ahora
 *   - "Parar" (rojo): actualiza la fila abierta con ended_at = ahora + duracion
 *
 * Una fila en doa_project_time_entries = una sesion de trabajo.
 * Al iniciar: INSERT {project_id, project_number, started_at}.
 * Al parar: n8n busca la fila con ended_at=null y hace UPDATE {ended_at, duration_minutes}.
 *
 * El status se determina consultando la ultima fila del project:
 *   - Si ended_at IS NULL → timer corriendo (Iniciar deshabilitado, Parar is_active)
 *   - Si ended_at IS NOT NULL o no hay filas → parado (Iniciar is_active, Parar deshabilitado)
 *
 * Incluye Realtime para sincronizar entre users.
 * ============================================================================
 */
...
export function ProjectTimerButton({ proyectoId, numeroProyecto }: ProjectTimerButtonProps) {
  ...
  // Date/hora del started_at de la sesion abierta (para calcular elapsed)
  ...
  // --- Cargar status inicial: buscar ultima fila del project ---
  useEffect(() => {
    ...
    async function fetchLastEntry() {
      ...
      if (error) {
        console.error('Error cargando status del temporizador:', error)
      ...
      const last = data[0] as ProjectTimeEntry
      if (last.ended_at === null) {
        // Sesion abierta
        setTimerState('running')
        setStartTime(new Date(last.started_at))
      } else {
        setTimerState('stopped')
        setStartTime(null)
      }
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        {
          ...
          filter: `project_id=eq.${proyectoId}`,
        },
        (payload) => {
          const entry = payload.new as ProjectTimeEntry
          if (entry.ended_at === null) {
            setTimerState('running')
            setStartTime(new Date(entry.started_at))
          }
        },
      )
      .on(
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/webhooks/time-tracking', {
        method: 'POST',
