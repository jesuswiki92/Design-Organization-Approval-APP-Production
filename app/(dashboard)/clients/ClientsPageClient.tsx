...
import type { ClienteWithContactos, ClienteContacto } from '@/types/database'
...
function ClientDetailPanel({
  ...
  const principalContact = client.contactos.find((c) => c.es_principal) ?? client.contactos[0] ?? null
  ...
  return (
    <div className="w-[400px] shrink-0 flex flex-col bg-[#1A1D27] border-l border-[#2A2D3E] overflow-auto">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-[#2A2D3E]">
        <div>
          <h2 className="text-base font-semibold text-[#E8E9F0]">{client.nombre}</h2>
          {client.cif_vat && (
            <p className="text-xs text-[#6B7280] mt-0.5 font-mono">{client.cif_vat}</p>
          )}
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex flex-col gap-5 px-5 py-4">
              ...
              {client.contactos.map((contact) => (
                ...
                >
                  <div className="flex items-center gap-2.5 mb-2">
                    ...
                    <div>
                      <p className="text-sm font-medium text-[#E8E9F0]">
                        {contact.nombre} {contact.apellidos ?? ''}
                      </p>
                      {contact.cargo && (
                        <p className="text-xs text-[#6B7280]">{contact.cargo}</p>
                      )}
                    </div>
                  </div>
        ...
        {client.notas && (
          <div className="flex flex-col gap-2.5">
            ...
            <p className="text-sm text-[#6B7280] whitespace-pre-wrap">{client.notas}</p>
          </div>
        )}
      </div>
    </div>
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <p className="text-sm font-medium text-[#E8E9F0]">{client.nombre}</p>
                            {client.cif_vat && (
                              <p className="text-xs text-[#6B7280] font-mono">{client.cif_vat}</p>
                            )}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {client.tipo_cliente ? (
                          <span className="text-xs px-2 py-0.5 rounded bg-[#6366F1]/10 text-[#6366F1] border border-[#6366F1]/20">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-[#6B7280]">{principalName}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {client.telefono ? (
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-sm text-[#6B7280]">{client.telefono}</span>
                          </div>
                        ) : (
                          <span className="text-xs text-[#6B7280]">—</span>
                        )}
                        ...
                        <span
                          className={cn(
                            ...
                            client.activo
                              ...
                              : 'text-gray-500 bg-gray-700/20 border-gray-600/20',
                          )}
                        >
                          <span
                            className={cn(
                              'w-1.5 h-1.5 rounded-full',
                              client.activo ? 'bg-emerald-400' : 'bg-gray-500',
                            )}
                          />
