...
This change stays inside the current App Router pattern: server `page.tsx` files fetch Supabase data, then pass serializable props to client components. `Quotations` will stop rendering an empty board and will hydrate the first lane from `doa_consultas_entrantes`. `Clientes` will enrich the existing client list with `doa_clientes_contactos` and surface those contacts in the left detail panel.
...
|---|---|---|---|
| Fetch on server | Load `doa_consultas_entrantes` in `app/(dashboard)/quotations/page.tsx` and contacts in `app/(dashboard)/clients/page.tsx`. | Client-side `useEffect`, or a new data layer first. | The repo already separates server pages from client UI. This preserves SSR and keeps the change aligned with current patterns. |
| Map incoming requests into cards | Convert `ConsultaEntrante` rows into board cards and inject them only into `entrada_recibida`. Keep real backend `estado` as metadata and navigate to `/quotations/incoming/[id]`. | Reuse `/quotations/[id]` or introduce a second board model. | `/quotations/incoming/[id]` already exists and is the correct operational route. Reusing the board shell minimizes UI churn. |
| Build contact grouping in server page | Query `doa_clientes_datos_generales` and `doa_clientes_contactos` separately, then group by `cliente_id` before passing `ClienteWithContactos[]` to the client. | Relational Supabase select or a dedicated contacts store. | Separate queries are explicit, easy to debug, and avoid coupling the UI to a more complex relational shape while the feature is still being introduced. |

## Data Flow

### Quotations

`page.tsx` -> Supabase `doa_consultas_entrantes` -> `toIncomingQuery()` -> incoming card adapter -> `QuotationStatesBoard`
...
```text
Supabase rows -> toIncomingQuery() -> incoming card adapter -> lane "entrada_recibida" -> /quotations/incoming/[id]
```

### Clientes

`page.tsx` -> clients query -> contacts query -> group by `cliente_id` -> `ClientsPageClient`

The client detail panel receives the selected `ClienteWithContactos` entry. The left block renders the main client data first and then a compact contacts section ordered by `es_principal DESC, activo DESC, created_at ASC`.
...
sequenceDiagram
    ...
    P->>S: select doa_consultas_entrantes
    S-->>P: ConsultaEntrante[]
    P->>B: pass initial cards for entrada_recibida
    B-->>B: render card href=/quotations/incoming/[id]

    C->>S: select doa_clientes_datos_generales
    C->>S: select doa_clientes_contactos
    S-->>C: Cliente[] + ClienteContacto[]
    C-->>C: group by cliente_id and render contacts in detail panel
```
...
| `app/(dashboard)/quotations/quotation-board-data.ts` | Modify | Add an incoming-request card mapper and lane injection for `entrada_recibida`. |
| `app/(dashboard)/clients/page.tsx` | Modify | Fetch contacts and build `ClienteWithContactos[]` before rendering the client surface. |
| `app/(dashboard)/clients/ClientsPageClient.tsx` | Modify | Render grouped contacts inside the left detail block. |
| `types/database.ts` | Reuse | Keep using `ClienteContacto` and `ClienteWithContactos` as the contract. |
...
type ClientsPageData = ClienteWithContactos[]
```
...
| Integration | Server pages return hydrated props | Confirm `Quotations` and `Clientes` render with real Supabase data. |
