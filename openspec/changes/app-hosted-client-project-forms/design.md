...
## Data Flow
    ...
    Quotations send-client
            |
            v
    app/api/consultas/[id]/send-client/route.ts
            |
            | create or reuse link row
            v
    doa_consultas_form_links (token hash + consultation_id + variant)
            |
    ...
    app/api/forms/[token]/route.ts
            ...
            v
    doa_consultas_form_responses + doa_consultas_entrantes
            |
            v
...
| `app/(dashboard)/quotations/page.tsx` | Modify | Add the `Formularios` button/entry that links to the internal catalog. |
| `app/(dashboard)/quotations/QuotationsClient.tsx` | Modify | Accept and render the catalog navigation entry if needed by layout. |
| `app/api/consultas/[id]/send-client/route.ts` | Modify | Add `formUrl` / `formToken` to the webhook payload and upsert the form link row. |
...
Unknown-client submissions SHALL persist both a JSON payload and normalized fields for company/contact/project data. Known-client submissions SHALL persist only the technical section plus the consultation linkage. `dominio_email` SHALL be derived from the sender email, not entered in the form.

Conditional logic:
- `Referencia interna del proyecto` MUST be shown only when `Tipo de trabajo = Modificacion a proyecto existente`.
