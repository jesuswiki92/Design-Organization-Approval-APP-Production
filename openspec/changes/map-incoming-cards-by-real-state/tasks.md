...
- [ ] 1.1 Add `app/api/consultas/[id]/state/route.ts` to validate allowed incoming-query states and persist `doa_consultas_entrantes.estado`.
...
- [ ] 2.1 Refactor `app/(dashboard)/quotations/quotation-board-data.ts` to stop forcing every incoming card into `entrada_recibida` and map each card to the lane for its real normalized state.
...
- [ ] 3.1 Verify the quotations board with a real `doa_consultas_entrantes` row in each supported state and confirm each card lands in the matching lane.
