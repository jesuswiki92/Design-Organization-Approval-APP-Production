...
/**
 * Reemplaza un placeholder literal en el XML de Word tolerando que el texto
...
/**
 * Genera un fichero .docx a partir de una plantilla .dotx aplicando
...
export async function generarDocxDesdeTemplate(opts: {
  ...
  for (const [key, value] of Object.entries(replacements)) {
    ...
    xml = next
    if (count === 0) {
      // No abortamos: puede que esa plantilla simplemente no tenga ese placeholder.
      console.warn(
        `[plantillas-docx] Placeholder "${key}" no encontrado en ${dotxPath}`,
      )
    }
  }
  ...
  // Nota: el tipo MIME no se cambia; basta con la extension .docx al escribir
  // para que Word lo abra como documento editable.
  const out = await zip.generateAsync({ type: 'nodebuffer' })
  await writeFile(destDocxPath, out)
}

// ─── Resolucion de nombre de fichero .dotx ──────────────────────────────────

let cachedTemplateDir: string | null = null
let cachedFiles: string[] | null = null

/**
 * Escanea una vez el directorio de plantillas y cachea la lista de ficheros.
...
export async function primeTemplateCache(plantillasRoot: string): Promise<void> {
  ...
  try {
    ...
    console.error('[plantillas-docx] No se pudo leer el directorio de plantillas:', err)
    cachedFiles = []
    cachedTemplateDir = plantillasRoot
  }
}
...
/**
 * Mapea un codigo de plantilla (ej. "G12-01") al nombre real del fichero
 * `.dotx` en el directorio de plantillas cacheado. Devuelve `null` si no
 * se encuentra.
 *
 * El matching es tolerante: busca una ocurrencia que empiece por `${code} `
 * o por `${code}_` (algunas plantillas antiguas pueden usar otro separador).
 */
